...
const Workflows = () => {
    ...
    const { workflows, error, loading, filters, pagination, refetchWorkflows, setFilters, setPage } = useWorkflows();
    ...
    const COLUMNS = [
        { header: "#", accessor: "id" },
        { header: t('workflowsManagement.workflows_table.code'), accessor: "code" },
        { header: t('workflowsManagement.workflows_table.name'), accessor: "name" },
        { header: t('workflowsManagement.workflows_table.description'), accessor: "description" },
        {
            header: t('workflowsManagement.workflows_table.date'),
            accessor: (row) =>
                lang === 'ar'
    ...
    const onDelete = async (selected) => {
        try {
            ...
            const res = await workflowService.deleteWorkflow(selected.id);
            toast.success(t('workflowsManagement.deleteWorkflowSuccessStatus'))
            refetchWorkflows();
    ...
    const handleBulkAction = (ids, actionType) => {
        handleBulk({
            serviceObject: workflowService,
            serviceMethod: "bulkActions",
            actionType: actionType,
            params: [ids, actionType],
            onSuccess: (data) => { toast.success(data?.message); refetchWorkflows() },
    ...
    return (
        <>
            <div className={`w-full flex flex-col gap-4`}>
                <BreedCrump breed_title={t('layouts.sideBar.settings.workflows')} />
                ...
                <div className={`bg-white shadow-md rounded-2xl flex flex-col gap-2`}>
                    <div className={`w-full flex flex-row justify-between items-center flex-wrap p-4`}>
                        ...
                        <div className='flex gap-2'>
                            {/* <CheckUserPermission allowedPermission={"workflows.export"}>
                            ...
                            <CheckUserPermission allowedPermission={"workflows.create"}>
                    ...
                    <DataTable columns={COLUMNS} data={workflows} loading={loading} selected_rows={selectedRows} onChangeSelected={setSelectedRows} onPageChange={setPage} pagination={pagination} renderRowActions={(row) => {
                        const canAnyPermission = hasAnyPermission(user?.permissions, ["workflows.edit", "workflows.delete"], user?.role?.name?.toLowerCase());
                        return canAnyPermission && (
                            <div className={`flex flex-row gap-2 items-center`}>
                                <CheckUserPermission allowedPermission={"workflows.edit"}>
                                ...
                                <CheckUserPermission allowedPermission={"workflows.delete"}>
