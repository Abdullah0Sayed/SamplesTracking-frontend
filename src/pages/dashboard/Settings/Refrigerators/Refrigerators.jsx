...
const Refrigerators = () => {
    ...
    const { refrigerators, error, loading, filters, pagination, refetchRefrigerators, setFilters, setPage } = useRefrigerators();
    ...
    const COLUMNS = [
        { header: "#", accessor: "id" },
        { header: t('refrigeratorManagement.refrigerators_table.name'), accessor: "name" },
        { header: t('refrigeratorManagement.refrigerators_table.description'), accessor: "description" },
        {
            header: t('refrigeratorManagement.refrigerators_table.date'),
            accessor: (row) =>
                lang === 'ar'
    ...
    const onDelete = async (selected) => {
        try {
            ...
            const res = await refrigeratorService.deleteRefrigeratorById(selected.id);
            toast.success(t('refrigeratorManagement.deleteRefrigeratorSuccessStatus'))
            refetchRefrigerators();
    ...
    const handleBulkAction = (ids, actionType) => {
        handleBulk({
            ...
            onSuccess: (data) => { toast.success(data?.message); refetchRefrigerators() },
                ...
                <div className={`bg-white shadow-md rounded-2xl flex flex-col gap-2`}>
                    <div className={`w-full flex flex-row justify-between items-center flex-wrap p-4`}>
                        ...
                        <div className='flex gap-2'>
                            ...
                            <CheckUserPermission allowedPermission={"refrigerators.create"}>
                    ...
                    <DataTable columns={COLUMNS} data={refrigerators} loading={loading} selected_rows={selectedRows} onChangeSelected={setSelectedRows} onPageChange={setPage} pagination={pagination} renderRowActions={(row) => {
                        const canAnyPermission = hasAnyPermission(user?.permissions, ["refrigerators.edit", "refrigerators.delete"], user?.role?.name?.toLowerCase());
                        return canAnyPermission && (
                            <div className={`flex flex-row gap-2 items-center`}>

                                <CheckUserPermission allowedPermission={"refrigerators.edit"}>
                                ...
                                <CheckUserPermission allowedPermission={"refrigerators.delete"}>
