...
const Freezers = () => {
    ...
    const { freezers, error, loading, filters, pagination, refetchFreezers, setFilters, setPage } = useFreezers();
    ...
    const COLUMNS = [
        { header: "#", accessor: "id" },
        { header: t('freezerManagement.freezers_table.name'), accessor: "name" },
        { header: t('freezerManagement.freezers_table.description'), accessor: "description" },
        {
            header: t('freezerManagement.freezers_table.date'),
            accessor: (row) =>
                lang === 'ar'
    ...
    const onDelete = async (selected) => {
        try {
            ...
            const res = await freezerService.deleteFreezerById(selected.id);
            toast.success(t('freezerManagement.deleteFreezerSuccessStatus'))
            refetchFreezers();
    ...
    const handleBulkAction = (ids, actionType) => {
        handleBulk({
            serviceObject: freezerService,
            serviceMethod: "bulkActions",
            actionType: actionType,
            params: [ids, actionType],
            onSuccess: (data) => { toast.success(data?.message); refetchFreezers() },
    ...
    return (
        <>
            <div className={`w-full flex flex-col gap-4`}>
                <BreedCrump breed_title={t('layouts.sideBar.settings.freezers')} />
                ...
                <div className={`bg-white shadow-md rounded-2xl flex flex-col gap-2`}>
                    <div className={`w-full flex flex-row justify-between items-center flex-wrap p-4`}>
                        ...
                        <div className='flex gap-2'>
                            {/* <CheckUserPermission allowedPermission={"freezers.export"}>
                            ...
                            <CheckUserPermission allowedPermission={"freezers.create"}>
                                <Link
                                    to={'add'}
                                ...
                                >
                                    ...
                                    {t('freezerManagement.addFreezer')}
                                </Link>
                            </CheckUserPermission>
                    ...
                    <DataTable columns={COLUMNS} data={freezers} loading={loading} selected_rows={selectedRows} onChangeSelected={setSelectedRows} onPageChange={setPage} pagination={pagination} renderRowActions={(row) => {
                        const canAnyPermission = hasAnyPermission(user?.permissions, ["freezers.edit", "freezers.delete"], user?.role?.name?.toLowerCase());
                        return canAnyPermission && (
                            <div className={`flex flex-row gap-2 items-center`}>

                                <CheckUserPermission allowedPermission={"freezers.edit"}>
                                ...
                                <CheckUserPermission allowedPermission={"freezers.delete"}>
