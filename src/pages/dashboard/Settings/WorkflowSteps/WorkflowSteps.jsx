...
const WorkflowSteps = () => {
  ...
  const COLUMNS = [
    { header: "#", accessor: "id" },
    {
      header: t("workflowStepsManagement.workflowSteps_table.testType"),
      accessor: (row) => (
        <span>{row?.test_type?.code + " - " + row?.test_type?.name}</span>
      ),
    },
    {
      header: t("workflowStepsManagement.workflowSteps_table.department"),
      accessor: (row) => (
        <span>
    ...
    {
      header: t("workflowStepsManagement.workflowSteps_table.masterStep"),
      accessor: (row) => (
        <span>
    ...
    {
      header: t("workflowStepsManagement.workflowSteps_table.stepOrder"),
      accessor: "step_order",
    },
    {
      header: t("workflowStepsManagement.workflowSteps_table.isFinalStep"),
      accessor: (row) => (
        <span>
          {row?.is_final_step === 1 ? t("global.yes") : t("global.no")}
        </span>
      ),
    },
    {
      header: t("workflowStepsManagement.workflowSteps_table.date"),
      accessor: (row) =>
        lang === "ar"
  ...
  const onDelete = async (selected) => {
    try {
      ...
      const res = await workflowStepsService.deleteWorkflowStep(selected.id);
      toast.success(
        t("workflowStepsManagement.deleteWorkflowStepSuccessStatus"),
      );
      refetchWorkflowSteps();
              ...
              <CheckUserPermission allowedPermission={"test_codes.export"}>
                <div className="flex gap-2">
                  ...
                  >
                    <FaFileArrowDown
                      className="w-5 h-5"
                      title={`${lang === "en" ? "Export Admins CSV Sheet" : "تصدير شيت المسؤلين"}`}
                    />{" "}
                    {t("global.download")}
                  </button>
                </div>
