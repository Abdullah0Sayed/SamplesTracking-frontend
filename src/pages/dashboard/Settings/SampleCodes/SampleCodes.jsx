...
const SampleCodes = () => {
    ...
    const { sampleCodes, error, loading, filters, pagination, refetchSampleCodes, setFilters, setPage } = useSampleCodes();
    ...
    const COLUMNS = [
        { header: "#", accessor: "id" },
        { header: t('sampleCodesManagement.sample_codes_table.code'), accessor: "code" },
        { header: t('sampleCodesManagement.sample_codes_table.name'), accessor: "name" },
        {
            header: t('sampleCodesManagement.sample_codes_table.date'),
            accessor: (row) =>
                lang === 'ar'
    ...
    const onDelete = async (selected) => {
        try {
            ...
            const res = await sampleCodesService.deleteSampleCode(selected.id);
            toast.success(t('sampleCodesManagement.deleteSampleCodeSuccessStatus'))
            refetchSampleCodes();
    ...
    const handleBulkAction = (ids, actionType) => {
        handleBulk({
            serviceObject: sampleCodesService,
            serviceMethod: "bulkActions",
            actionType: actionType,
            params: [ids, actionType],
            onSuccess: (data) => { toast.success(data?.message); refetchSampleCodes() },
                ...
                <div className={`bg-white shadow-md rounded-2xl flex flex-col gap-2`}>
                    <div className={`w-full flex flex-row justify-between items-center flex-wrap p-4`}>
                        ...
                        <div className='flex gap-2'>
                            {/* <CheckUserPermission allowedPermission={"sample_codes.export"}>
                                ...
                                <div className="flex gap-2">
                                    <button
                                        ...
                                        <FaFileArrowDown className="w-5 h-5" title={`${lang === 'en' ? 'Export Sample Codes CSV Sheet' : 'تصدير شيت أكواد العينات'}`} /> {t('global.download')}
                                    </button>
                                </div>
                            </CheckUserPermission> */}


                            <CheckUserPermission allowedPermission={"sample_codes.create"}>
                    ...
                    <DataTable columns={COLUMNS} data={sampleCodes} loading={loading} selected_rows={selectedRows} onChangeSelected={setSelectedRows} onPageChange={setPage} pagination={pagination} renderRowActions={(row) => {
                        const canAnyPermission = hasAnyPermission(user?.permissions, ["sample_codes.edit", "sample_codes.delete"], user?.role?.name?.toLowerCase());
                        return canAnyPermission && (
                            <div className={`flex flex-row gap-2 items-center`}>

                                <CheckUserPermission allowedPermission={"sample_codes.edit"}>
                                ...
                                <CheckUserPermission allowedPermission={"sample_codes.delete"}>
