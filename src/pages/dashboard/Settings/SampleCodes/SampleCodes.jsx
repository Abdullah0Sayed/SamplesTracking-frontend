...
const SampleCodes = () => {
  ...
  const COLUMNS = [
    { header: "#", accessor: "id" },
    {
      header: t("sampleCodesManagement.sample_codes_table.code"),
      accessor: "code",
    },
    {
      header: t("sampleCodesManagement.sample_codes_table.name"),
      accessor: "name",
    },
    {
              ...
              <CheckUserPermission allowedPermission={"sample_codes.export"}>
                <div className="flex gap-2">
                  ...
                  >
                    <FaFileArrowDown
                      className="w-5 h-5"
                      title={`${lang === "en" ? "Export Sample Types CSV Sheet" : "تصدير شيت أنواع العينات"}`}
                    />{" "}
                    {t("global.download")}
                  </button>
                </div>
          ...
          <DataTable
            ...
            renderRowActions={(row) => {
              const canAnyPermission = hasAnyPermission(
                user?.permissions,
                ["sample_codes.edit", "sample_codes.delete"],
              ...
              return (
                canAnyPermission && (
                  <div className={`flex flex-row gap-2 items-center`}>
                    <CheckUserPermission
                      allowedPermission={"sample_codes.edit"}
                    >
                      <Link
                        to={`${row.id}/edit`}
                    ...
                    <CheckUserPermission
                      allowedPermission={"sample_codes.delete"}
                    >
                      <button
                        className={`w-8 h-8 text-xl rounded-lg bg-grey-2 text-grey-1 flex items-center justify-center hover:bg-grey-5 transition`}
                        onClick={() => {
