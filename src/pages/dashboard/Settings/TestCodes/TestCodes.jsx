...
const TestCodes = () => {
    ...
    const { testCodes, error, loading, filters, pagination, refetchTestCodes, setFilters, setPage } = useTestCodes();
    ...
    const COLUMNS = [
        { header: "#", accessor: "id" },
        { header: t('testCodesManagement.test_codes_table.code'), accessor: "code" },
        { header: t('testCodesManagement.test_codes_table.name'), accessor: "name" },
        {
            header: t('testCodesManagement.test_codes_table.date'),
            accessor: (row) =>
                lang === 'ar'
    ...
    const onDelete = async (selected) => {
        try {
            ...
            const res = await testCodesService.deleteTestCode(selected.id);
    ...
    const handleBulkAction = (ids, actionType) => {
        handleBulk({
            serviceObject: TestCodes,
            serviceMethod: "bulkActions",
            actionType: actionType,
            params: [ids, actionType],
            onSuccess: (data) => { toast.success(data?.message); refetchTestCodes() },
                ...
                <div className={`bg-white shadow-md rounded-2xl flex flex-col gap-2`}>
                    <div className={`w-full flex flex-row justify-between items-center flex-wrap p-4`}>
                        ...
                        <div className='flex gap-2'>
                            ...
                            <CheckUserPermission allowedPermission={"test_codes.create"}>
                    ...
                    <DataTable columns={COLUMNS} data={testCodes} loading={loading} selected_rows={selectedRows} onChangeSelected={setSelectedRows} onPageChange={setPage} pagination={pagination} renderRowActions={(row) => {
                        const canAnyPermission = hasAnyPermission(user?.permissions, ["test_codes.edit", "test_codes.delete"], user?.role?.name?.toLowerCase());
                        return canAnyPermission && (
                            <div className={`flex flex-row gap-2 items-center`}>
                                <CheckUserPermission allowedPermission={"test_codes.edit"}>
                                ...
                                <CheckUserPermission allowedPermission={"test_codes.delete"}>
