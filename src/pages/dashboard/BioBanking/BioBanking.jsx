...
const BioBanking = () => {
  ...
  const COLUMNS = [
    { header: "#", accessor: "id" },
    {
      header: t("bioBankingManagement.bioBanking_table.subBarCode"),
      accessor: "sub_barcode",
    },
    // {
    //   header: t("bioBankingManagement.bioBanking_table.sample"),
    //   accessor: "sample",
    // },
    {
      header: t("bioBankingManagement.bioBanking_table.itemType"),
      accessor: "item_type",
    },
    {
      header: t("bioBankingManagement.bioBanking_table.temperature"),
      accessor: "storage_temp",
    },
    {
      header: t("bioBankingManagement.bioBanking_table.volume"),
      accessor: "volume",
    },
    {
      header: t("bioBankingManagement.bioBanking_table.tubeType"),
      accessor: "tube_type",
    },
    {
      header: t("bioBankingManagement.bioBanking_table.storageLocation"),
      accessor: (row) => row?.storage_location_id?.unit_name,
    },
    {
      header: t("bioBankingManagement.bioBanking_table.shelf"),
      accessor: "shelf_id",
    },
    {
      header: t("bioBankingManagement.bioBanking_table.rack"),
      accessor: "rack_id",
    },
    {
      header: t("bioBankingManagement.bioBanking_table.box"),
      accessor: "box_id",
    },
    {
      header: t("bioBankingManagement.bioBanking_table.position"),
      accessor: "position_id",
    },
    {
      header: t("bioBankingManagement.bioBanking_table.notes"),
      accessor: "notes",
    },
    {
      header: t("bioBankingManagement.bioBanking_table.sample"),
              ...
              <CheckUserPermission allowedPermission={"bioBanking.export"}>
                <div className="flex gap-2">
                  ...
                  >
                    <FaFileArrowDown
                      className="w-5 h-5"
                      title={`${lang === "en" ? "Export BioBanking CSV Sheet" : "تصدير شيت النتائج الحيوية"}`}
                    />{" "}
                    {t("global.download")}
                  </button>
                </div>
          ...
          <DataTable
            ...
            renderRowActions={(row) => {
              const canAnyPermission = hasAnyPermission(
                user?.permissions,
                ["bioBanking.edit", "bioBanking.delete"],
              ...
              return (
                canAnyPermission && (
                  <div className={`flex flex-row gap-2 items-center`}>
                    ...
                    <CheckUserPermission allowedPermission={"bioBanking.show"}>
                      <Link
                        to={`${row.id}/profile`}
                        className={`w-8 h-8 text-xl rounded-lg bg-grey-2 text-grey-1 flex items-center justify-center hover:bg-grey-5 transition`}
                      >
                    ...
                    <CheckUserPermission
                      allowedPermission={"bioBanking.delete"}
                    >
                      <button
                        className={`w-8 h-8 text-xl rounded-lg bg-grey-2 text-grey-1 flex items-center justify-center hover:bg-grey-5 transition`}
                        onClick={() => {
