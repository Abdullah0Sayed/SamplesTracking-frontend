    ...
    const onsubmit = async (data) => {
        try {
            if (roleId) {
                ...
                const res = await rolesService.addRole(data);
            ...
            <form className="flex flex-col gap-4" onSubmit={handleSubmit(onsubmit)}>
                ...
                <div className="w-full bg-white p-4 grid grid-cols-2 gap-4 rounded-2xl">
                    ...
                    <div className="col-span-2 flex flex-col gap-2">
                        ...
                        <input
                            {...register("name", {
                                validate: (value) => /^[^\u0600-\u06FF]*$/.test(value) || `غير مسموح بالحروف العربية`
                                ,
                                onChange: (e) => e.target.value = e.target.value.replace(/[\u0600-\u06FF]/g, "")
                        ...
                        />
                        <p className={`text-sm text-black/20 font-semibold flex items-center gap-1`}> <BiInfoCircle /> {t('accountsManagement.roles.rolesFields.acceptOnlyEnLetters')}</p>
                        {errors['name'] && <p className={`text-sm text-red-300 font-semibold flex items-center gap-1`}> <BiInfoCircle /> {errors['name'].message} </p>}
                    </div>
                        ...
                        >
                            ...
                            <AnimatePresence>
                                {openGroups.includes(index) && (
                                    ...
                                    >
                                        {permissionsByGroupObject[collection_name].map((permission) => (
                                            <label
                                                key={permission.id}
                                                className="flex items-center gap-3 cursor-pointer"
                                            >
