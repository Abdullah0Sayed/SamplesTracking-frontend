...
const ChangePassword = () => {
  ...
  return (
    <div className={`flex flex-col gap-4`}>
      <Header />
      <div
        className={`w-11/12 mx-auto rounded-2xl border shadow-md flex flex-row mb-4`}
      >
        <HeroBrand
          heroBrandTitle={t("auth.login.heroLoginHeading")}
          heroBrandDescription={t("auth.resetPassword.heroLoginDescription")}
        />
        <div
          className={`w-full min-h-screen h-full flex flex-col gap-8 justify-center items-center`}
        >
          ...
          <div className={`w-5/6 flex flex-col gap-0`}>
            <Form
              ...
              submit_label={t("auth.forgetPassword.send")}
              onSubmit={onSubmit}
              showCancelBtn={false}
            />
            <div className={`w-full flex flex-row justify-center items-center`}>
              <Link
                to={"/"}
                className={`px-4 text-xs text-primary-color opacity-50 transition-all hover:opacity-100`}
              >
                {t("auth.forgetPassword.rememberPassword")}{" "}
                <span className={`text-second-color underline`}>
                  {t("auth.login.title")}
                </span>
              </Link>
            </div>
