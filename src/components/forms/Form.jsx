...
const Form = ({ fields, initial_values, onSubmit, onCancel, submit_label, showCancelBtn, gridLayout = 'grid-cols-1' }) => {
    ...
    const { register, handleSubmit, control, setValue, reset, formState: { isSubmitting, errors } } =
    ...
    return (
        <form encType="multipart/form-data" onSubmit={handleSubmit(onSubmit)} className="w-full flex flex-col gap-8 p-4">
            <div className={`w-full grid ${gridLayout} gap-4`}>
                ...
                {fields.map((field, index) => {
                    ...
                    if (field.type === 'single-select') {
                        return (
                            <div key={index} className={`${field.full_width ? 'col-span-2' : 'flex flex-col gap-4'}`}>
                                ...
                                <Controller
                                    ...
                                    render={({ field: fi }) => (
                                        <Select
                                            ...
                                            isClearable
                                            value={(field.options || []).find(opt => opt.value === fi.value)}
                                            styles={{
                                                control: base => ({
                                                    ...base,
                                                    fontSize: '0.875rem'
                                ...
                                field.type === 'file' ? (
                                    ...
                                    field.type === 'textArea' ? (
                                        <textarea
                                            ...
                                            className="w-full border rounded-md px-4 py-2 focus:outline-none focus:shadow"
                                        ></textarea>
                                    ) : field.name === 'phone' ? (<div className={`relative flex justify-between w-full  `}>

                                        <input
                                            type={field.type}
                                            {...register(field.name, { validation, onChange: (e) => e.target.value = e.target.value.replace(/\D/, "") })}
                                            placeholder={field.placeholder}

                                            className="w-full px-4 py-2 border rounded-md focus:outline-none focus:shadow"
                                        />
                                        <div className={`absolute w-16 h-full ${lang === 'en' ? 'right-0' : 'left-0'} flex justify-center items-center text-grey-5 border rounded `}>
                                            +966
                                        </div>
