...
const Header = () => {
  ...
  const [authedAlerts, setAuthedAlerts] = useState([]);
  const [openNotifications, setOpenNotifications] = useState(false);

  const [isReadNotification, setIsReadNotification] = useState(null);
  ...
  const fetchAuthedNotifications = useMemo(async () => {
    try {
      if (!user) return;
      const { data } = await authService.getMyNotifications({
        is_read: 0,
      });
      setAuthedAlerts(data?.notifications);
  ...
  return (
    <>
      <header
        className={`w-full h-20 flex justify-between items-center p-2 shadow-md bg-white`}
      >
        {baseName.includes("auth") && (
          <div className={`w-full fixclick-logo`}>
            <img
              src={"/logo.png"}
              alt={`molTracking-logo`}
        ...
        {baseName.includes("dashboard") && (
          <>
            <div
              className={`w-full mobile:hidden tablet:hidden smallScreen:hidden medScreen:hidden flex flex-col gap-2`}
            >
            ...
            {/* زرار Menu للهاتف */}
            <div
              className="text-2xl cursor-pointer mobile:block tablet:block smallScreen:block medScreen:block hidden"
              onClick={(e) => {
        ...
        <div
          className={`w-full grid grid-cols-[auto,auto,auto] justify-end items-center gap-4`}
        >
          {/* Language Menu */}
          <div className={`w-full flex flex-row mobile:hidden`}>
            <div className={`relative`} onClick={(e) => e.stopPropagation()}>
              <button
                ...
                onClick={() => openMenu(setOpenLanguageMenu)}
              >
              ...
              {openLanguageMenu && (
                ...
                >
                  <label
                    className={`flex gap-2 cursor-pointer px-2 py-2 rounded-lg hover:bg-second-color/50`}
                    onClick={() => swipeLang("ar")}
                  >
                    <input type={`checkbox`} className={`peer hidden`} />
                    <span
                      className={`w-6 h-6 rounded-full border-2 ${lang === "ar" ? "bg-second-color border-second-color" : "bg-white border-gray-400"} flex items-center justify-center`}
                    ></span>
                  ...
                  >
                    <input type={`checkbox`} className={`peer hidden`} />
                    <span
                      className={`w-6 h-6 rounded-full border-2 ${lang === "en" ? "bg-second-color border-second-color" : "bg-white border-gray-400"} flex items-center justify-center`}
                    ></span>
                    <BiCheck className={`absolute w-6 h-6 text-white block`} />
          ...
          {baseName.includes("dashboard") && (
            <div className="relative menu">
              <div
                ...
                onClick={() => openMenu(setOpenNotifications)}
              >
                <FiBell className="text-2xl" />
                {user && authedAlerts && (
                  <span className="absolute -top-2 -right-2 w-4 h-4 text-xs bg-red-500 text-white rounded-full flex items-center justify-center">
                    {
                      authedAlerts.filter((alert) => alert.read_at === null)
                        .length
                    }
                  </span>
                )}
              ...
              {openNotifications && (
                <div
                  className={`
                    ${`absolute mt-12 w-72 top-2 ${lang === "ar" ? "left-0" : "right-0"}`}
                ...
                >
                  ...
                  {user &&
                    authedAlerts?.map((alert, index) => (
                      <NotificationCard
                        key={index}
                        alertTitle={alert?.data?.title}
                        alertDescription={alert?.data?.description}
                        isReading={alert.is_read}
                      />
                    ))}
                </div>
              )}
          ...
          {baseName.includes("dashboard") && (
            <div
              className={`w-full relative flex flex-row gap-2`}
              onClick={(e) => e.stopPropagation()}
            >
              {/* Authed Profile Image */}
              {baseName.includes("dashboard") && (
                <div
                  className={`w-12 h-12 rounded-full bg-primary-color flex justify-center items-center text-white font-bold`}
                >
              ...
              <div
                ...
                onClick={() => openMenu(setOpenAccountControl)}
              >
              ...
              {openAccountControl && (
                <div
                  className={`absolute ${lang === "ar" ? "left-0" : "right-0"} mt-14 w-56 bg-white border shadow-md px-2 py-4 rounded-xl flex flex-col gap-2 z-50`}
                >
      ...
      {isOpenOverlayMenu && (
        ...
        >
          <div onClick={(e) => e.stopPropagation()}>
            ...
            <Sidebar isMobile onClose={() => setIsOpenOverLayMenu(false)} />
          </div>
        </div>
      )}
    </>
