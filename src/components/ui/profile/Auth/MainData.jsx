...
const MainData = () => {
    ...
    const fetchAuthedUser = async (user) => {
        ...
        try {
            if (!user) return;
            const { data } = await authService.getAuthedUser(user?.id);
    ...
    return (
        <div className={`flex flex-row flex-1 items-center justify-between`}>
            ...
            <Form gridLayout='grid-cols-1' fields={FIELDS} initial_values={INITIAL_VALUES} submit_label={t('global.save')} onSubmit={onSubmit} showCancelBtn={true} />
