...
const SampleTestSteps = ({ SampleTests = [] }) => {
    ...
    const COLUMNS = [
        { header: "#", accessor: "id" },
        { header: t('samplesManagement.sample_tests_status_table.test_code'), accessor: "test_code" },
        { header: t('samplesManagement.sample_tests_status_table.results'), accessor: "results" },
        { header: t('samplesManagement.sample_tests_status_table.notes'), accessor: "notes" },
        { header: t('samplesManagement.sample_tests_status_table.status'), accessor: (row) => <span className={`px-2 py-1 rounded-full text-xs ${spanColoring(row.current_status)}`}>{row.status}</span> },
        { header: t('samplesManagement.sample_tests_status_table.created_by'), accessor: (row) => <span className={`px-2 py-1 rounded-full text-xs }`}>{row.created_by}</span> },

        {
            header: t('samplesManagement.sample_tests_status_table.performed_at'),
            accessor: (row) =>
                lang === 'ar'
                    ? formattedDate(row.performed_at).toLocaleDateString("ar-EG", {
                        day: "numeric",
                        month: "long",
                        year: "numeric",
                    })
                    : formattedDate(row.performed_at).toLocaleDateString("en-US", {
                        day: "numeric",
                        month: "long",
                        year: "numeric",
                    })
    ...
    const SAMPLE_TEST_STATUS_FIELDS = useMemo(() => [
        {
            name: 'status',
            type: 'single-select',
            placeholder: t('samplesManagement.samplesTestsStatusFields.statusPlaceholder'),
        ...
        },
        {
            name: 'result_file',
            type: 'file',
            placeholder: t('samplesManagement.samplesTestsStatusFields.resultFilePlaceholder'),
        ...
        },
        {
            name: 'results',
            type: 'textArea',
            placeholder: t('samplesManagement.samplesTestsStatusFields.descriptionPlaceholder'),
        ...
        },
        {
            name: 'notes',
            type: 'textArea',
            placeholder: t('samplesManagement.samplesTestsStatusFields.notesPlaceholder'),
    ...
    return (
        <>
            <div className='w-full'>
                <DataTable columns={COLUMNS} data={SampleTests} loading={loading} selected_rows={selectedRows} onChangeSelected={setSelectedRows} renderRowActions={(row) => (
                    <div className={`flex flex-row gap-2 items-center`}>
                        ...
                        >
                            <BiEdit title={`${lang === 'en' ? 'Update Test Status' : 'تحديث حالة الاختبار'}`} />
                        </button>
                    </div>
                )} />
            </div>
...

            {
                isEditModalOpen && <Modal onClose={() => setIsEditModalOpen(false)}>
                    ...
                    <div className={`w-full flex flex-col gap-4`}>
                        <p className={`text-lg font-semibold`}>{t('samplesManagement.editSampleTestStatus')} {selectedRow.test_code} </p>
